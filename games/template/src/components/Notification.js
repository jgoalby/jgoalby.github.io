...
export default class Notification extends Phaser.GameObjects.Container {
  ...
  constructor(scene) {
    ...
    this.panel = this.scene.add.nineslice(width * 2, height - (100 + 20), "panel", 0, 400, 100, 32, 32, 32, 32);
    this.panel.setOrigin(0.5, 1);

    this.scene.tweens.add({targets: this.panel, x: width / 2, ease: 'quart.out', duration: 1000, hold: 1500, yoyo: true, completeDelay: 1000, oncomplete: () => { this.destroy() }});
