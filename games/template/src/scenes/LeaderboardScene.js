...
export default class LeaderboardScene extends BaseScene {
  constructor(config) {
    if (!config) { config = {} }
    config.key = config.key || Constants.SCENES.LEADERBOARD_SCENE;
    super(config);
  ...
  createScene() {
    ...
    this.button = new Button(this, { shortcut: 'X', label: 'Menu', actionFn: () => { this.gotoScene(Constants.SCENES.MENU_SCENE) } });
  }
